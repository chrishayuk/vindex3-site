...
export const ENTITIES: Entity[] = [
	...
	{
		id: "token-mixer",
		names: ["token mixer", "token-mixer", "token mixing", "mixer", "token mixer programme"],
		display: "TOKEN MIXER",
		five: "how a layer moves information sideways",
		role: "The part of a layer that moves information between positions — the job attention does, and the job a recurrence does instead. A model need not use the same one in every layer.",
		...
		relations: [
			...
			{ rel: "specialised_by", to: "attention" },
			{ rel: "specialised_by", to: "linear-attention" },
			{ rel: "contrasted_with", to: "ffn" },
		],
		href: "/anatomy",
		explorer: "DESCRIBE layer.0.mixer",
	},
	{
		id: "attention",
		...
		relations: [
			...
			{ rel: "contains", to: "o" },
			{ rel: "belongs_to", to: "layer" },
			{ rel: "belongs_to", to: "token-mixer" },
			{ rel: "specialised_by", to: "gated-attention" },
		],
		href: "/anatomy",
	...
	{
		id: "model-program",
		names: ["model program", "operation program", "operation plan", "op plan", "componentopplan"],
		...
		relations: [
			{ rel: "read_by", to: "closure-e" },
			{ rel: "generalises", to: "execution-surface-e" },
			{ rel: "enables", to: "deletion-invariant-e" },
		],
		href: "/execution",
	},
	{
		...
		names: ["compiler boundary", "source compiler", "front end", "architecture-aware ingestion"],
		display: "THE COMPILER BOUNDARY",
		five: "family knowledge compiles away here",
		role: "The one place architecture-specific judgment is allowed to live: the source compiler, which reads a family's reference semantics and compiles them into generic VINDEX3 IR.",
	...
	{
		id: "causal-control",
		names: ["causal control", "causal controls", "mutation control", "causal mutation control"],
		display: "CAUSAL CONTROLS",
		five: "mutate the fact, watch execution",
		role: "The proof that the persisted IR is in charge: mutate a stored fact — a scale, a layer policy, a judged semantic — and the computation must change or refuse; where mutation changes nothing, a default was hiding.",
		detail: "Three controls, each touching only the persisted graph: a scalar authority control (change the query scale, diverge from layer zero), a layer-policy control (flip one layer's position policy, only that layer diverges), and an op-semantic control (remove a judged gate, refusal names the primitive). They are the search for hidden defaults — and they found none. Closure proves the program is complete; parity proves it is correct; the controls prove the IR is in charge.",
		group: "format",
		relations: [
			{ rel: "completes", to: "closure-e" },
			{ rel: "proves", to: "deletion-invariant-e" },
		],
		href: "/execution",
	},
	{
		id: "residency",
		names: ["residency", "residence", "residency contract", "residency plan", "resident set"],
		display: "RESIDENCY",
		five: "where the bytes live now",
		role: "The third axis of the container model: identity says what an object is, a representation says how it is encoded, residency says where the useful bytes — and execution state — live at this moment.",
		detail: "Residency is operational, never semantic: the same container means the same thing mmap-cold on NVMe or resident in unified memory. The pinned division: the format may carry portable residency facts — access class, zero-copy capability, evictability, co-use grouping, state lifetimes — while placement belongs to a runtime plan that is never persisted. A device name in the format would age with the machine, not the model. And residency is measured, not assumed: sparse does not imply a small resident set.",
		group: "format",
		relations: [
			{ rel: "sits_below", to: "representation-e" },
			{ rel: "excluded_from", to: "weight-classes" },
		],
		href: "/container",
	},
	{
		id: "gated-attention",
		names: ["gated attention", "gated_attention", "output gate", "output-gated attention", "attention output gate"],
		display: "GATED ATTENTION",
		five: "softmax attention, plus a gate",
		role: "Softmax attention whose output is modulated by a learned gate before it rejoins the residual stream — the gate is added to the operator, not substituted for it.",
		...
		relations: [
			{ rel: "belongs_to", to: "attention" },
			{ rel: "contrasted_with", to: "gated-deltanet" },
		],
		href: "/execution",
		explorer: "DESCRIBE layer.3.mixer",
	},
	{
		id: "linear-attention",
